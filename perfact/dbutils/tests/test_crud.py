...
def test_update(conn):
    ...
    update(conn=conn, table="appuser", ident=ident, payload=update_payload)
    # Verify update
    res = conn.execute(
...
def test_update_no_auditmode(conn):
    ...
    update(
        conn=conn, table="appuser", ident=ident, payload=update_payload, auditmode=False
    )
    # Verify update
    res = conn.execute(
        "select * from appuser where appuser_id = %(appuser_id)s",
...
def test_update_in_ident(conn):
    ...
    update_payload = {
        "appuser_name": "Franklin",
        "appuser_fullname": "Frank Miller Jr.",
    }
    update(conn=conn, table="appuser", ident=ident, payload=update_payload)
    # Verify update
    res = conn.execute(
...
def _setup_read_testdata(conn):
    ...
    """
    payload = {"appuser_name": "Syril", "appuser_fullname": "Syril Karn"}
    create(conn=conn, table="appuser", payload=payload)
    payload = {"appuser_name": "Luthen", "appuser_fullname": "Luthen Rael"}
    create(conn=conn, table="appuser", payload=payload)
...
def test_read_ident(conn):
    ...
    selected = read(conn=conn, table="appuser", ident={"appuser_name": "Luthen"})
    assert len(selected) == 1
    # Test with explicitly setting ident to None
    selected = read(conn=conn, table="appuser", ident=None)
    assert len(selected) == 2
...
def test_read_where(conn):
    ...
    selected = read(
        ...
        table="appuser",
        where={
            "stmt": ("appuser_modtime <= now() and appuser_name = %(appuser_name)s"),
            "payload": {"appuser_name": "Luthen"},
        },
    )
    assert len(selected) == 1
    ...
    selected = read(
        ...
        table="appuser",
        where={
            "stmt": ("appuser_modtime <= now() and appuser_id < 10"),
        },
    )
    assert len(selected) == 2
    ...
    selected = read(
        ...
        table="appuser",
        where={
            "stmt": ("appuser_name = %(appuser_name)s"),
            "payload": {"appuser_name": "Cassian"},
        },
    )
    assert not selected
...
def test_read_orderby(conn):
    ...
    selected = read(conn=conn, table="appuser", orderby=["appuser_name"])
    assert selected[0]["appuser_name"] == "Luthen"
    # Test order by with tuple
    selected = read(conn=conn, table="appuser", orderby=[("appuser_name", "desc")])
    ...
    with pytest.raises(ValueError):
        selected = read(
            conn=conn, table="appuser", orderby=[("appuser_name", "; select 1;")]
        )
    # Test incomplete tuple
    with pytest.raises(ValueError):
        selected = read(conn=conn, table="appuser", orderby=[tuple("appuser_name")])
